...
class KeybindReference:
    def __init__(self):
        ...
        self.descriptions_file = self.hypr_config / "scripts/resources/keybind_descriptions.yaml"
        self.keybind_files = [
        ...
        for config_file in self.keybind_files:
            ...
            for pattern in patterns:
                ...
                for modifier, key, action in matches:
                    ...
                    keybind_str = f"{modifier}+{key}" if modifier != "" else f" + {key}"
    ...
    def show_tui(self):
        ...
        # Title
        title = Text("Hyprland Keybind Reference", style="bold magenta", justify="center")
        ...
        for category, binds in self.keybinds.items():
            ...
            table = Table(title=f"{icon} {category}", box=box.ROUNDED, title_style=f"bold {color}")
    ...
    def show_rofi(self):
        ...
        cmd = [
            'rofi', '-dmenu',
            '-mesg', ' Hyprland Keybind Reference',
            '-i',  # Case-insensitive
            '-theme-str', 'window { width: 75%; height: 75%; }',
